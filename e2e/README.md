...
- Démarrer automatiquement le serveur de développement (`npm run dev`) avant les tests
...
1. **Base de données** : Le test nécessite une base de données accessible (SQLite local ou PostgreSQL selon votre configuration)
   - **IMPORTANT** : Assurez-vous que le schéma de la base de données est à jour avant de lancer les tests :
     ...
     # ou
     npm run db:setup:production  # Pour PostgreSQL
     ```
2. **Serveur de développement** : Playwright démarre automatiquement `npm run dev` avant les tests (ou réutilise un serveur existant)
...
Assurez-vous que le serveur de développement peut démarrer correctement. Vous pouvez tester manuellement avec `npm run dev`.
