...
export async function POST(request: NextRequest) {
  try {
    ...
    if (useBlobStorage && !isBlobConfigured) {
              logger.warn('API UPLOAD - Blob demandé mais non configuré, utilisation du dossier local en fallback');
      ...
      try {
        ...
        if (originalImage && originalImage.type.startsWith('image/')) {
          if (originalImage.size <= 15 * 1024 * 1024) {
            ...
            logger.warn(`API UPLOAD - Fichier original trop volumineux: ${originalImage.size} bytes`);
          }
        }
            ...
            try {
              ...
              const extension = originalImage.name.includes('.')
                ...
                : 'jpg';
                      ...
                      logger.debug(`API UPLOAD - Image originale uploadée vers Vercel Blob: ${originalKey}`);
                    } catch (error) {
    ...
    return NextResponse.json(
      { error: 'Aucun système de stockage configuré' },
      { status: 503 }
    );
          } catch (error) {
