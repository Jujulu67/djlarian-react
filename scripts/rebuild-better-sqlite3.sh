...
# Script pour recompiler better-sqlite3 sans utiliser les variables npm dépréciées
# Utilise npm rebuild (méthode recommandée) qui gère mieux les chemins avec espaces
...
# Utiliser npm rebuild (méthode recommandée par les bonnes pratiques)
...
if npm rebuild better-sqlite3 2>&1; then
    ...
    echo "⚠️  Erreur avec npm rebuild, tentative avec node-gyp..."
